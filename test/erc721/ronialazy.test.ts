...
async function deploy() {
  ...
  let factory = await ethers.getContractFactory("Ronia721Lazy", minter)
...
describe("Ronia721Lazy", function () {
  it("Should deploy", async function () {
    const signers = await ethers.getSigners();
    const minter = signers[0].address;

    const LazyNFT = await ethers.getContractFactory("Ronia721Lazy");
  ...
  it("Should redeem an NFT from a signed voucher", async function () {
    ...
    const voucher = await lazyMinter.createVoucher(1, "ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi", 0, expiredAt)
  ...
  it("Should redeem if payment is >= minPrice", async function () {
    ...
    const expiredAt = Math.floor(Date.now() / 1000) + (60 * 60 * 24);
    const voucher = await lazyMinter.createVoucher(1, "ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi", minPrice, expiredAt)
  ...
  it("Should fail to redeem if voucher is expired", async function () {
    const { contract, redeemerContract, redeemer, minter } = await deploy()
